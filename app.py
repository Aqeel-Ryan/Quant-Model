...
if selected_task == "1. Stock Summary":
    ...
    code = """
        ...
        varList = ['T10Y3M', 'DGS10', 'OBMMIJUMBO30YF',  # term premium 10yr-3mon, 30 yr mortgage jumbo loan
                ...
                'BAMLH0A0HYM2EY', # ICE BofA High Yield Index Effective Yield
                'DGS1',          # 1-Year Treasury Constant Maturity Rate
                'DCOILWTICO',    # West Texas Intermediate Crude Oil Price
                'DHHNGSP',       # Henry Hub Natural Gas Spot Price
                'EMVELECTGOVRN', # Equity Market Volatility Tracker for Elections and Political Governance
                'ECIWAG'         # Employment Cost Index: Wages and Salaries for Private Industry Workers 
    ...
    # Title
    st.title("Benchmarch Study using GARCH-t")
    st.code("""
def garch(param, *args):
    ...
    st.header("GARCH-t Model Analysis Results")

    st.subheader("Returns vs Predictions")
    st.image("data/logvspred.png")

    st.subheader("Standardized Residuals")
    st.image("data/resiTime.png")

    st.subheader("Volatility Over Time")
    st.image("data/volaOvertime.png")
